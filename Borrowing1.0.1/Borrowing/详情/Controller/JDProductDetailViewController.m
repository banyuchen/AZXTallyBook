...
//@property (nonatomic, strong) DBPopAnimator *animator;
...
- (void)viewDidLoad {
    ...
    webView.size = CGSizeMake(kWindowW, 100);
...
//    }];
    ...
    [[JDStatisticsManager sharedInstance] addloanShopPVUVWithAppId:ThirdAppId userId:[DBSave account].ID position:@"7" token:@"" pattern:@"10" type:@"2" productId:self.productM.ID callSucceedBack:^(id  _Nullable responseObject) {
...
#pragma mark - 立即申请
- (void)immediatelyBtnClick:(UIButton *)sender
{
    NZLog(@"立即申请");
...
//        [self.navigationController pushViewController:vc animated:YES];
        ...
        self.popAnimator = [[PopAnimator alloc]initWithCoverFrame:coverFrame presentedFrame:presentedFrame startPoint:CGPointMake(0.5, 1.0) startTransform:CGAffineTransformMakeScale(0.1, 0.1) endTransform:CGAffineTransformMakeScale(0.0001, 0.0001)];
        
        JDDetailLoginViewController *vc = [[JDDetailLoginViewController alloc] init];
...
- (void)wzWebViewDidFinishLoad:(UIWebView *)webView
{
    ...
    self.tableView.tableFooterView.height = frame.size.height+kMargin;
    self.tableView.tableFooterView = webView;
    NZLog(@"%f--------------%f",webView.scrollView.contentSize.height,frame.size.height+kMargin);
...
- (void)viewDidLoad
{
    ...
    if (![self.isApplyNow isEqualToString:@"YES"]) {
